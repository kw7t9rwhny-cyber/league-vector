...
function validateBudget(b) {
  objectExactKeys(b, ["max_worker_runs"]);
...
function validateWorkItem(w) {
  objectExactKeys(w, ["schema_version","work_item_id","objective","role","risk","input_identity","context_refs","allowed_actions","forbidden_actions","expected_terminal_result","qa_requirement","founder_gate","confidentiality","budget","replay_identity"]);
  ...
  if (w.role!=="research") fail("invalid_role",w.role); if (!RISKS.has(w.risk)) fail("invalid_risk",w.risk);
  ...
  if (w.expected_terminal_result!==RESULT_SCHEMA_VERSION) fail("invalid_expected_terminal_result",w.expected_terminal_result);
  if (w.qa_requirement!=="one") fail("invalid_qa_requirement",w.qa_requirement);
  if (w.founder_gate!==true) fail("invalid_founder_gate");
  if (w.confidentiality!=="public") fail("unsupported_confidentiality",w.confidentiality);
...
function validateSubstance(role, s) {
  ...
  if (!ROLES.has(role)) fail("invalid_role",role);
