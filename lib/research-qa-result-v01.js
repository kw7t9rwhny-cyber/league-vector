...
function validateRunProvenance(result, run, {repository, now=new Date().toISOString()}={}) {
  ...
  if(!["queued","in_progress","completed"].includes(run.status)) fail("invalid_worker_run_status");
  return true;
}
...
module.exports={WORKFLOW_PATHS,buildAuthoritativeResult,validateTerminalResult,validateRunProvenance,proveExactlyOneTerminal};
