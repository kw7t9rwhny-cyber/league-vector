...
function validateRunProvenance(result, run, {repository, now=new Date().toISOString(), requireCompleted=true}={}) {
  ...
  if(requireCompleted){
    // Downstream authority must be monotonic: only independently observed completed/success can qualify.
    if(run.status!=="completed") fail("worker_run_not_completed");
    if(run.conclusion!=="success") fail("worker_run_not_successful");
  } else {
    // Local persistence may correlate its own still-running worker, but completed failures never validate.
...
function validateResearchCompletionWake(event){
  const run=event?.workflow_run;
  if(!run||typeof run!=="object") fail("missing_research_completion_event");
  if(run.status!=="completed") fail("research_completion_not_completed");
  if(run.conclusion!=="success") fail("research_completion_not_successful");
  if(run.event!=="workflow_dispatch") fail("wrong_worker_event");
  if(run.path!==WORKFLOW_PATHS.research) fail("wrong_worker_workflow");
  if(run.head_branch!=="main") fail("wrong_worker_ref");
  const match=/^RQA Research issue #([1-9][0-9]*)$/.exec(run.display_title||"");
  if(!match) fail("invalid_research_completion_run_name");
...
module.exports={WORKFLOW_PATHS,buildAuthoritativeResult,validateTerminalResult,proofArtifactName,validateRunProvenance,validateResearchCompletionWake,validateRunProofArtifact,proveExactlyOneTerminal};
