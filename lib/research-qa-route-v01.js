...
function checkBudget({budget, worker_runs_used}) {
  ...
  // max_worker_runs is the maximum number of worker dispatches that may exist.
  // A new dispatch is allowed iff the proposed total (used + 1) does not exceed max.
  if (worker_runs_used+1>budget.max_worker_runs) return {ok:false,reason:"run_limit_exhausted"};
  return {ok:true};
}
function blockedBudget(work_item,usage){const budget=checkBudget({budget:work_item.budget,...usage});return budget.ok?null:{action:"STOP",disposition:"BLOCKED",reason:budget.reason,founder_gate:true};}
function route({work_item,research_results=[],qa_results=[],dispatches=[],usage}) {
  ...
  try { rr=proveExactlyOneTerminal(research_results,researchExpectation); } catch(e) {
    if (e.code==="missing_terminal_result") {
      ...
      const limit=blockedBudget(work_item,usage); if(limit)return limit;
  ...
  // Existing authoritative QA terminal state wins before any budget decision about hypothetical future work.
