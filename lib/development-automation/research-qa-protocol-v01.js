...
const TRUSTED_RESULT_WRITER = Object.freeze({ login: 'github-actions[bot]', type: 'Bot' });
...
function validateBudget(budget) {
  ...
  if (!Number.isInteger(budget.max_worker_runs) || budget.max_worker_runs < 1 || budget.max_worker_runs > 4) return fail('invalid_max_worker_runs');
  if (!Number.isInteger(budget.max_actions_runtime_minutes) || budget.max_actions_runtime_minutes < 1 || budget.max_actions_runtime_minutes > 10) return fail('invalid_max_actions_runtime');
  if (budget.max_ai_cost_or_credits !== undefined && budget.max_ai_cost_or_credits !== null && (!Number.isFinite(budget.max_ai_cost_or_credits) || budget.max_ai_cost_or_credits < 0)) return fail('invalid_ai_budget');
  return pass();
}
...
function computeResultId({ workItemId, role, roleInstanceId, workerRunId, runAttempt, inputIdentity, upstreamResultIds }) {
  return sha256(canonicalJson({ work_item_id: workItemId, role, role_instance_id: roleInstanceId, worker_run_id: workerRunId, run_attempt: runAttempt, input_identity: inputIdentity, upstream_result_ids: upstreamResultIds }));
}
function computeDispatchIdentity({ workItem, role, roleInstanceId, upstreamResultIds = [] }) {
  return sha256(canonicalJson({ schema: CLAIM_SCHEMA_VERSION, replay_identity: workItem.replay_identity, work_item_id: workItem.work_item_id, role, role_instance_id: roleInstanceId, input_identity: workItem.input_identity, upstream_result_ids: upstreamResultIds }));
}
...
function buildTerminalResult({ workItem, role, roleInstanceId, workerRunId, runAttempt, upstreamResultIds = [], substance, writerIdentity = TRUSTED_RESULT_WRITER, createdAt, telemetry }) {
  ...
  const resultId = computeResultId({ workItemId: workItem.work_item_id, role, roleInstanceId, workerRunId, runAttempt, inputIdentity: workItem.input_identity, upstreamResultIds });
  return {
...
function validateTerminalResult(result, { workItem, expectedRole, expectedRoleInstanceId, expectedWorkerRunId, expectedRunAttempt = 1, expectedUpstreamResultIds = [], commentAuthor, now = Date.now(), maxAgeMs = 60 * 60 * 1000 }) {
  const required = ['schema_version','result_id','work_item_id','role','role_instance_id','worker_run_id','run_attempt','input_identity','upstream_result_ids','writer_identity','created_at','terminal_state','substance','telemetry'];
  ...
  if (!isPlainObject(result.writer_identity) || result.writer_identity.login !== TRUSTED_RESULT_WRITER.login || result.writer_identity.type !== TRUSTED_RESULT_WRITER.type) return fail('untrusted_writer_claim');
  if (commentAuthor && (commentAuthor.login !== TRUSTED_RESULT_WRITER.login || commentAuthor.type !== TRUSTED_RESULT_WRITER.type)) return fail('untrusted_comment_author');
  ...
  const expectedId = computeResultId({ workItemId: result.work_item_id, role: result.role, roleInstanceId: result.role_instance_id, workerRunId: result.worker_run_id, runAttempt: result.run_attempt, inputIdentity: result.input_identity, upstreamResultIds: result.upstream_result_ids });
...
function validateDispatchClaim(claim, { workItem, role, roleInstanceId, upstreamResultIds = [], commentAuthor }) {
  const required = ['schema_version','dispatch_identity','work_item_id','role','role_instance_id','input_identity','upstream_result_ids','created_at','claim_state'];
  ...
  if (commentAuthor && (commentAuthor.login !== TRUSTED_RESULT_WRITER.login || commentAuthor.type !== TRUSTED_RESULT_WRITER.type)) return fail('untrusted_dispatch_claim_author');
  return pass();
}
...
module.exports = {
  ...
  buildDispatchClaim, validateDispatchClaim, computeDispatchIdentity, computeResultId,
