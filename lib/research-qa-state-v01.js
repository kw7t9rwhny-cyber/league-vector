...
function trustedDurableWriter(comment){ return comment?.user?.login==="github-actions[bot]" && comment?.user?.type==="Bot" && comment?.user?.id===41898282; }
function parseAuthoritativeResults(comments){
  const out=[]; for(const x of comments||[]){ const parsed=parseTaggedRecord(x.body,MARKERS.result); if(!parsed||!trustedDurableWriter(x)) continue; validateTerminalResult(parsed); out.push(parsed); } return out;
}
...
function parseDispatches(comments){
  const out=[]; for(const x of comments||[]){ const parsed=parseTaggedRecord(x.body,MARKERS.dispatch); if(!parsed||!trustedDurableWriter(x)) continue; validateDispatchRecord(parsed); out.push(parsed); } return out;
}
...
module.exports={MARKERS,reconcileWrite,telemetryRecord,taggedRecord,parseTaggedRecord,parseWorkItemIssue,trustedDurableWriter,parseAuthoritativeResults,validateDispatchRecord,parseDispatches,buildDispatchRecord,validateWorkItemCreatorAuthority};
