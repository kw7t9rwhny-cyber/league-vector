...
function validateTaskContract(contract, {now = new Date(), expectedRepository = EXPECTED_REPOSITORY} = {}) {
  ...
  exactStringArray(contract.prohibited_path_patterns, "prohibited_path_patterns", {min: REQUIRED_PROHIBITED_PATH_PATTERNS.length, max: 100});
  for (const required of REQUIRED_PROHIBITED_PATH_PATTERNS) {
    if (!contract.prohibited_path_patterns.includes(required)) throw new Error("prohibited_path_baseline_missing");
  }
  ...
  if (typeof contract.maximum_provider_cost_usd !== "number" || !Number.isFinite(contract.maximum_provider_cost_usd) || contract.maximum_provider_cost_usd <= 0 || contract.maximum_provider_cost_usd > MAX_PROVIDER_COST_USD) {
  ...
  exactStringArray(contract.prohibited_actions, "prohibited_actions", {min: REQUIRED_PROHIBITED_ACTIONS.length, max: 100});
  for (const required of REQUIRED_PROHIBITED_ACTIONS) {
    if (!contract.prohibited_actions.includes(required)) throw new Error("prohibited_action_baseline_missing");
  }
  ...
  exactStringArray(contract.stop_conditions, "stop_conditions", {min: REQUIRED_STOP_CONDITIONS.length, max: 100});
  for (const required of REQUIRED_STOP_CONDITIONS) {
    if (!contract.stop_conditions.includes(required)) throw new Error("stop_condition_baseline_missing");
  }
...
function validateDeterministicEvidence(evidence, expected) {
  exactKeys(evidence, [
    "schema_version", "task_contract_identity", "implementation_run_identity", "candidate_pr_number",
  ...
  for (const field of ["task_contract_identity", "implementation_run_identity", "candidate_pr_number", "expected_candidate_commit", "expected_candidate_tree", "expected_base_commit"]) {
  ...
  if (!/^\d+$/.test(String(evidence.workflow_run_id)) || !/^\d+$/.test(String(evidence.run_attempt))) throw new Error("evidence_run_identity_invalid");
