...
const VALIDATION_WORKFLOW_PATH = ".github/workflows/product-task-pipeline-v01-exact-head.yml";
const VALIDATION_WORKFLOW_REF = `${EXPECTED_REPOSITORY}/${VALIDATION_WORKFLOW_PATH}@refs/heads/main`;
...
const MARKERS = Object.freeze({
  ...
  validationDispatch: "<!-- LEAGUE_VECTOR_PRODUCT_TASK_VALIDATION_DISPATCH_V0.1 -->",
...
function isProhibitedPath(file) {
  ...
  if (segments.some((segment) => segment.includes("customer"))) return true;
...
function assertNoDuplicateJsonMembers(source) {
  if (typeof source !== "string") throw new Error("tagged_record_malformed");
  let index = 0;
  const syntax = () => { throw new Error("tagged_record_malformed"); };
  const skipWhitespace = () => {
    while (index < source.length && /[\t\n\r ]/.test(source[index])) index += 1;
  };
  const parseString = () => {
    if (source[index] !== '"') syntax();
    const start = index;
    index += 1;
    while (index < source.length) {
      ...
      if (character === '"') {
        index += 1;
        try { return JSON.parse(source.slice(start, index)); } catch { syntax(); }
      }
      if (character === "\\") {
        index += 1;
        if (index >= source.length) syntax();
        if (source[index] === "u") {
          if (!/^[0-9a-fA-F]{4}$/.test(source.slice(index + 1, index + 5))) syntax();
          index += 5;
          continue;
        }
      }
  ...
  const parseValue = () => {
    ...
    if (source[index] === "{") {
      ...
      while (index < source.length) {
        ...
        if (keys.has(key)) throw new Error("tagged_record_duplicate_member");
        keys.add(key);
        skipWhitespace();
        if (source[index] !== ":") syntax();
        index += 1;
...
function parseTaggedRecords(text, marker) {
  ...
  for (let match; (match = regex.exec(text));) {
    try {
      assertNoDuplicateJsonMembers(match[1]);
      out.push(JSON.parse(match[1]));
    } catch (error) {
      if (error && error.message === "tagged_record_duplicate_member") throw error;
      throw new Error("tagged_record_malformed");
    }
  }
  return out;
}
...
function assertUniqueExecution(comments, idempotencyIdentity) {
  const records = [
    ...
    ...parseTrustedRecords(comments, MARKERS.validationDispatch),
...
function assertValidationDispatchAvailable(comments, implementationRunIdentity) {
  const implementation = parseTrustedRecords(comments, MARKERS.implementation).filter((record) => record.implementation_run_identity === implementationRunIdentity);
  const dispatches = parseTrustedRecords(comments, MARKERS.validationDispatch).filter((record) => record.implementation_run_identity === implementationRunIdentity);
  if (implementation.length !== 0 || dispatches.length !== 0) throw new Error("validation_dispatch_already_recorded");
  return true;
}

function deriveValidationDispatchIdentity(binding) {
  return sha256(canonical({
    task_id: binding.task_id,
    task_contract_identity: binding.task_contract_identity,
    implementation_run_identity: binding.implementation_run_identity,
    implementation_workflow_run_id: String(binding.implementation_workflow_run_id),
    implementation_workflow_run_attempt: String(binding.implementation_workflow_run_attempt),
    candidate_pr_number: binding.candidate_pr_number,
    ...
    validation_workflow_path: VALIDATION_WORKFLOW_PATH
  }));
}

function assertValidationDispatchAuthority(comments, expected) {
  const records = parseTrustedRecords(comments, MARKERS.validationDispatch).filter((record) => record.implementation_run_identity === expected.implementation_run_identity);
  if (records.length === 0) throw new Error("trusted_validation_dispatch_missing");
  if (records.length !== 1) throw new Error("trusted_validation_dispatch_duplicate");
  const record = records[0];
  exactKeys(record, [
    "schema_version", "record_type", "validation_dispatch_identity", "task_id",
    "task_contract_identity", "idempotency_identity", "implementation_run_identity",
    "implementation_workflow_run_id", "implementation_workflow_run_attempt",
    "validation_workflow_id", "validation_workflow_path", "validation_workflow_ref",
    "validation_workflow_run_id", "validation_workflow_run_attempt", "validation_actor",
    "validation_triggering_actor", "candidate_pr_number", "candidate_branch", "base_commit",
    "candidate_commit", "candidate_tree", "changed_paths", "created_at"
  ], "validation_dispatch");
  exactKeys(record.validation_actor, ["login", "id", "type"], "validation_dispatch_actor");
  exactKeys(record.validation_triggering_actor, ["login", "id", "type"], "validation_dispatch_triggering_actor");
  const exact = record.schema_version === EXECUTION_SCHEMA_VERSION &&
    record.record_type === "VALIDATION_DISPATCH" &&
    record.task_id === expected.task_id &&
    record.task_contract_identity === expected.task_contract_identity &&
    record.idempotency_identity === expected.idempotency_identity &&
    record.implementation_run_identity === expected.implementation_run_identity &&
    String(record.implementation_workflow_run_id) === String(expected.implementation_workflow_run_id) &&
    String(record.implementation_workflow_run_attempt) === String(expected.implementation_workflow_run_attempt) &&
    record.validation_dispatch_identity === expected.validation_dispatch_identity &&
    String(record.validation_workflow_run_id) === String(expected.workflow_run_id) &&
    String(record.validation_workflow_run_attempt) === String(expected.run_attempt) &&
    /^\d+$/.test(String(record.validation_workflow_id)) &&
    record.validation_workflow_path === VALIDATION_WORKFLOW_PATH &&
    record.validation_workflow_ref === VALIDATION_WORKFLOW_REF &&
    canonical(record.validation_actor) === canonical(ACTIONS_BOT) &&
    canonical(record.validation_triggering_actor) === canonical(ACTIONS_BOT) &&
    record.validation_actor.login === expected.validation_actor &&
    record.validation_triggering_actor.login === expected.validation_triggering_actor &&
    record.candidate_pr_number === expected.candidate_pr_number &&
    record.candidate_branch === expected.expected_candidate_branch &&
    record.base_commit === expected.expected_base_commit &&
    record.candidate_commit === expected.expected_candidate_commit &&
    record.candidate_tree === expected.expected_candidate_tree &&
    canonical(record.changed_paths) === canonical(expected.expected_changed_paths);
  if (!exact) throw new Error("trusted_validation_dispatch_binding_mismatch");
  const derived = deriveValidationDispatchIdentity(record);
  if (derived !== record.validation_dispatch_identity) throw new Error("trusted_validation_dispatch_identity_mismatch");
  iso(record.created_at, "validation_dispatch_created_at");
  return record;
}
...
function validateDeterministicEvidence(evidence, expected) {
  exactKeys(evidence, [
    "schema_version", "task_contract_identity", "implementation_run_identity",
    "implementation_workflow_run_id", "implementation_workflow_run_attempt", "validation_dispatch_identity",
    "candidate_pr_number", "expected_candidate_branch",
    ...
    "command_results", "workflow_run_id", "run_attempt", "validation_actor",
    "validation_triggering_actor", "validation_workflow_path", "validation_workflow_ref",
    "validation_workflow_sha", "validation_event_name", "started_at", "ended_at",
  ...
  for (const field of ["task_contract_identity", "implementation_run_identity", "implementation_workflow_run_id", "implementation_workflow_run_attempt", "validation_dispatch_identity", "candidate_pr_number", "expected_candidate_branch", "expected_candidate_commit", "expected_candidate_tree", "expected_base_commit", "validation_actor", "validation_triggering_actor", "validation_workflow_ref", "validation_workflow_sha", "validation_event_name"]) {
  ...
  if (!SHA64.test(evidence.task_contract_identity) || !SHA64.test(evidence.implementation_run_identity) || !SHA64.test(evidence.validation_dispatch_identity) || !Number.isInteger(evidence.candidate_pr_number) || evidence.candidate_pr_number < 1 || !evidence.expected_candidate_branch.startsWith("agent/product-task-") || evidence.validation_actor !== ACTIONS_BOT.login || evidence.validation_triggering_actor !== ACTIONS_BOT.login || evidence.validation_workflow_path !== VALIDATION_WORKFLOW_PATH || evidence.validation_workflow_ref !== VALIDATION_WORKFLOW_REF || evidence.validation_workflow_sha !== evidence.expected_base_commit || evidence.validation_event_name !== "workflow_dispatch") throw new Error("evidence_binding_invalid");
...
module.exports = {
  ...
  assertValidationDispatchAvailable,
  deriveValidationDispatchIdentity,
  assertValidationDispatchAuthority,
