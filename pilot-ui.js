(() => {
  ...
  async function init() {
    try {
      ...
      const catalogResponse = await fetch("data/pilot/prior-runs.json", { cache: "no-store" });
      if (!catalogResponse.ok) throw Object.assign(new Error("The complete demo history catalog could not be loaded. Retry this page."), { code: "snapshot_load_failed" });
      snapshot = await A.read(raw, { format, run: requestedRun, priorCatalog: await catalogResponse.text() });
