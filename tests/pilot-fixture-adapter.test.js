...
const read = (raw = clone(), format = "dynasty-1qb", run = undefined) => A.read(raw, { format, run, now });
...
test("both formats preserve exact Snapshot identities, missing fields and immutable views", () => {
  const qb = read(), sf = read(clone(), "dynasty-superflex");
...
test("presentation reasons match the exact Snapshot template", () => {
  for (const format of A.FORMATS) {
    const v = read(clone(), format);
...
test("search covers the entire universe and approved aliases without identity merging", () => {
  const v = read(); assert.equal(A.search(v, "the LANTERN")[0].rank, 64);
...
test("neighbors are delegated to exact Snapshot same-position semantics", () => {
  const v = read(), group = v.players.filter(p => p.position === "WR" && p.rank);
...
test("comparison never manufactures a winner when Snapshot v1 has no pairwise rule", () => {
  const v = read();
...
]) test(`rejects ${name}`, () => { const v = read(), r = refs(v); mutate(r); assert.throws(() => A.compare(v, r, now), { code }); });

test("outdated, withdrawn and future runs use exact Snapshot lifecycle semantics", () => {
  const v = read(); assert.throws(() => A.compare(v, refs(v), Date.parse(v.reviewAt)), { code: "outdated_compare" });
  const raw = clone(); raw.publication.state = "WITHDRAWN"; raw.published_at = raw.generated_at;
  assert.throws(() => read(raw), { code: "revoked_snapshot" });
  assert.throws(() => A.read(fixture, { format: "dynasty-1qb", now: Date.parse("2026-09-01") }), { code: "unavailable_run" });
});

test("missing, unknown run and invalid format remain distinct", () => {
  assert.throws(() => read(null), { code: "no_snapshot" }); assert.throws(() => read(clone(), "invalid"), { code: "unsupported_format" });
  assert.throws(() => read(clone(), "dynasty-1qb", "missing"), { code: "unavailable_run" });
});

test("browser canonical parsing fails closed on duplicate JSON keys and noncanonical bytes", () => {
  const bytes = S.canonicalizeSnapshot(fixture);
  assert.throws(() => read(bytes.replace('{', '{"run_id":"duplicate",')), { code: "malformed_snapshot" });
  assert.throws(() => read(bytes + "\n"), { code: "malformed_snapshot" });
  assert.equal(read(bytes).runId, fixture.run_id);
});
...
]) test(`delegates malformed admission: ${name}`, () => { const raw = clone(); mutate(raw); assert.throws(() => read(raw), { code: "malformed_snapshot" }); });

test("a structurally valid REAL snapshot is still refused by this synthetic-only UI", () => {
  ...
  assert.throws(() => read(raw), { code: "malformed_snapshot" });
});

test("history and change facts are mapped from the validated Snapshot entry", () => {
  const v = read(current), alpha = v.players.find(p => p.id === "lv:synthetic:alpha");
  ...
  assert.equal(read(prior).players.find(p => p.id === "lv:synthetic:alpha").history.state, "FIRST_RUN");
});
