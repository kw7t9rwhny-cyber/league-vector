...
test("no workflow grants pull request, deployment, package, or contents write authority",()=>{for(const x of [controller,research,qa]){assert.doesNotMatch(x,/pull-requests:\s*write/);assert.doesNotMatch(x,/deployments:\s*write/);assert.doesNotMatch(x,/packages:\s*write/);assert.doesNotMatch(x,/contents:\s*write/)}});
...
test("QA persistence has read-only Actions provenance access and no downstream Actions write",()=>{assert.match(qa,/persist:[\s\S]*?actions:\s*read/);assert.doesNotMatch(qa,/persist:[\s\S]*?actions:\s*write/);assert.match(qa,/and STOP/)});
test("QA preflight can independently read Actions run metadata",()=>assert.match(qa,/preflight:[\s\S]*?actions:\s*read/));
test("production helper independently fetches asserted Actions run before eligibility",()=>{assert.match(helper,/\/actions\/runs\/\$\{encodeURIComponent\(result\.worker_run_id\)\}/);assert.match(helper,/validateRunProvenance/);assert.match(helper,/await verifyResults\(s\.results\)/);assert.match(helper,/await verifyResults\(rs\)/)});
test("model prompt receives positive and forbidden action envelopes",()=>{assert.match(helper,/Allowed actions:/);assert.match(helper,/Forbidden actions:/)});
