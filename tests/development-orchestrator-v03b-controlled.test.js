...
const TRUSTED={source:"mock",repository_full_name:"x/y",default_branch:"main",fork:false};
const EXEC_ENV={
  LEAGUE_VECTOR_ORCHESTRATOR_EXECUTE:"1",
  LEAGUE_VECTOR_STAGE3B_ACTIVATED:"1",
  LEAGUE_VECTOR_STAGE3B_FOUNDER_ACTIVATED:"1",
  ...
  GITHUB_HEAD_REPO_FORK:"false",
  GITHUB_RUN_ID:"12345"
};

function body(o={}) {
  const x={owner:"projection",risk:"high",status:"qa-passed",type:"feature",priority:"normal",integration_required:"yes",promotion_type:"none",promotion_authorized:"not-applicable",founder_decision_required:"no",founder_gate:"none",founder_decision:"not-required",dependencies:"None",...o};
...
class MockAdapter {
  ...
  find(n){ return this.data.prs.find(x=>x.number===Number(n)); }
  async addLabel(repo,n,label){ this.writes.push(`ADD:${label}`); if(this.failAt===this.writes.length) throw new Error("write_fail"); const p=this.find(n); if(!p.labels.includes(label)) p.labels.push(label); if(this.failAfterApplyAt===this.writes.length) throw new Error("ambiguous_after_apply"); }
  async removeLabel(repo,n,label){ this.writes.push(`REMOVE:${label}`); if(this.failAt===this.writes.length) throw new Error("write_fail"); const p=this.find(n); p.labels=p.labels.filter(x=>x!==label); if(this.failAfterApplyAt===this.writes.length) throw new Error("ambiguous_after_apply"); }
}
function preview(d,n=101){ return C.previewFrom(d,n); }
async function exec(d,n=101,o={}){
  const p=preview(d,n);
  const adapter=o.adapter||new MockAdapter(d,o);
  const result=await C.executeControlled({repository:o.repository||"x/y",token:"mock",targetPr:String(n),expectedFingerprint:o.fingerprint===undefined?p.replay_fingerprint:o.fingerprint,env:o.env||EXEC_ENV,adapter});
  return {p,adapter,result};
}

for (const bad of ["", "0", "-1", "all", "*", "queue", "1,2", "1 2", "1-3", "[1,2]", "1.5", "01"]) {
  test(`reject target PR input ${JSON.stringify(bad)}`,()=>assert.throws(()=>C.parseTargetPr(bad),/invalid_target_pr_number/));
}
test("accept exactly one positive integer PR",()=>assert.equal(C.parseTargetPr("123"),123));
test("wrong PR number fails closed",()=>assert.throws(()=>preview(data(),999),/target_pr_not_found/));
test("closed PR produces no live mutation disposition",()=>{ const p=preview(data(pr(101,{state:"closed"}))); assert.equal(p.stage3a_disposition,"NO_MUTATION"); });
test("missing Founder activation denies before writes",async()=>{ const env={...EXEC_ENV}; delete env.LEAGUE_VECTOR_STAGE3B_FOUNDER_ACTIVATED; const r=await exec(data(),101,{env}); assert.match(r.result.abort_reason,/founder_gate/); assert.deepEqual(r.adapter.writes,[]); });
test("Founder activation must be exactly 1",()=>assert.equal(C.founderActivationGate({...EXEC_ENV,LEAGUE_VECTOR_STAGE3B_FOUNDER_ACTIVATED:"true"}).allowed,false));
...
for (const [name,patch] of [
  ["tag main",{GITHUB_REF:"refs/tags/main",GITHUB_REF_TYPE:"tag"}],
  ["push event",{GITHUB_EVENT_NAME:"push"}],
  ["schedule event",{GITHUB_EVENT_NAME:"schedule"}],
  ["pull_request event",{GITHUB_EVENT_NAME:"pull_request"}],
  ["wrong default branch",{GITHUB_DEFAULT_BRANCH:"other"}],
  ["missing execute flag",{LEAGUE_VECTOR_ORCHESTRATOR_EXECUTE:"0"}],
  ["missing activation flag",{LEAGUE_VECTOR_STAGE3B_ACTIVATED:"0"}]
]) test(`${name} denies live execution`,async()=>{ const r=await exec(data(),101,{env:{...EXEC_ENV,...patch}}); assert.match(r.result.abort_reason||"",/execution_gate/); assert.deepEqual(r.adapter.writes,[]); });
test("noncanonical label is rejected",()=>{ const d=data(), {plan}=C.planForTarget(d,101); plan.mutations=[{operation:"ADD_LABEL",label:"arbitrary"}]; assert.equal(C.canonicalMutationsOnly(plan).valid,false); });
test("extra arbitrary mutation is rejected even beside canonical mutations",()=>{ const d=data(), {plan}=C.planForTarget(d,101); plan.mutations.push({operation:"ADD_LABEL",label:"ship-it"}); assert.equal(C.canonicalMutationsOnly(plan).valid,false); });
...
test("second identical execution cannot replay stale preview",async()=>{ const d=data(), p=preview(d), adapter=new MockAdapter(d); const first=await exec(d,101,{adapter,fingerprint:p.replay_fingerprint}); assert.equal(first.result.stage3b_audit.post_write_verification,"verified"); const second=await C.executeControlled({repository:"x/y",token:"mock",targetPr:"101",expectedFingerprint:p.replay_fingerprint,env:EXEC_ENV,adapter}); assert.equal(second.abort_reason,"preview_state_changed"); });
test("partial write triggers bounded rollback or manual review",async()=>{ const d=data(), adapter=new MockAdapter(d,{failAt:2}); const r=await exec(d,101,{adapter}); assert.ok(r.result.stage3b_audit.aborted_reason); assert.ok(["rolled-back-to-before-state","failed-or-partial"].includes(r.result.stage3b_audit.post_write_verification)); });
test("ambiguous write never claims success",async()=>{ const d=data(), adapter=new MockAdapter(d,{failAfterApplyAt:1}); const r=await exec(d,101,{adapter}); assert.ok(r.result.stage3b_audit.aborted_reason); assert.notEqual(r.result.stage3b_audit.post_write_verification,"verified"); });
test("rollback conflict preserves human state and requires manual review",async()=>{ const d=data(); let changed=false; const adapter=new MockAdapter(d,{failAt:2,onRead:(a,reads)=>{ if(reads>=4&&!changed){ changed=true; a.data.prs[0].labels.push("owner:core"); } }}); const r=await exec(d,101,{adapter}); assert.ok(r.result.stage3b_audit.manual_review_required); });
test("dry-run proof: preview path has no adapter writes",()=>{ const d=data(); const p=preview(d); assert.equal(p.authorization,false); assert.ok(p.exact_mutations.length>0); });

test("workflow is manual-only, one-target, least-privilege, and Founder-environment gated",()=>{
  ...
  for(const forbidden of ["pull_request_target:","schedule:","push:","pull_request:","issues: write\n\s+pull-requests: write","contents: write","actions: write","deployments: write","packages: write"]) assert.equal(yml.includes(forbidden),false,forbidden);
  assert.match(yml,/target_pr_number:/);
  assert.match(yml,/environment: stage3b-controlled-activation/);
  assert.match(yml,/issues: write/);
  assert.match(yml,/pull-requests: read/);
  assert.match(yml,/LEAGUE_VECTOR_STAGE3B_FOUNDER_ACTIVATED/);
  assert.equal((yml.match(/workflow_dispatch:/g)||[]).length,1);
});

test("real adapter surface contains label add/remove only beyond inherited reads",()=>{
  const proto=Object.getOwnPropertyNames(C.GitHubControlledLabelAdapter.prototype).sort();
  assert.deepEqual(proto,["addLabel","constructor","removeLabel"]);
});
