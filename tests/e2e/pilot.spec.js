...
test("history uses exact Snapshot current/prior contracts", async ({ page }) => {
  ...
  await page.route("**/data/pilot/prior-runs.json", route => route.fulfill({ body: JSON.stringify([S.canonicalizeSnapshot(prior)]), contentType: "application/json" }));
  ...
  await expect(page.getByText("Changed input evidence")).toHaveCount(2);
  await expect(page.getByText(/Other players’ ordering changed/)).toHaveCount(0);
  // Preserve the unchanged-input behavior using an authenticated successor whose
  // measurement meaning really is unchanged, including source and as-of metadata.
  const unchanged = structuredClone(current);
  unchanged.run_id = "synthetic-ui-unchanged-inputs";
  unchanged.sources = structuredClone(prior.sources);
  unchanged.valid_until = prior.valid_until;
  for (const format of S.FORMATS) for (const entry of unchanged.formats[format].entries) {
    const before = prior.formats[format].entries.find(p => p.player_id === entry.player_id);
    for (const fact of entry.facts) {
      const old = before.facts.find(f => f.field === fact.field);
      for (const key of ["value", "unit", "period", "as_of", "source_id"]) fact[key] = structuredClone(old[key]);
      fact.run_id = unchanged.run_id;
    }
    entry.history.changed_fact_refs = [];
  }
  await S.validateSnapshotCatalog([prior, unchanged]);
  await page.unroute("**/data/pilot/synthetic-run.json");
  await mock(page, r => { for (const key of Object.keys(r)) delete r[key]; Object.assign(r, unchanged); });
  await page.goto(`/player.html?id=lv:synthetic:charlie&format=dynasty-1qb&run=${unchanged.run_id}`);
...
test("history without a complete authentic predecessor blocks every surface", async ({ page }) => {
  const current = require("../../fixtures/ranking-snapshot-v1/synthetic-current.json");
  const prior = require("../../fixtures/ranking-snapshot-v1/synthetic-prior.json");
  const forged = structuredClone(current);
  forged.previous_run.artifact_id = "sha256:" + "0".repeat(64);
  for (const [snapshot, catalog] of [[current, []], [forged, [S.canonicalizeSnapshot(prior)]]]) {
    await page.unroute("**/data/pilot/synthetic-run.json");
    await page.unroute("**/data/pilot/prior-runs.json");
    await mock(page, r => { for (const key of Object.keys(r)) delete r[key]; Object.assign(r, snapshot); });
    await page.route("**/data/pilot/prior-runs.json", route => route.fulfill({ body: JSON.stringify(catalog), contentType: "application/json" }));
    for (const url of ["/rankings.html", "/player.html?id=lv:synthetic:alpha", "/compare.html?a=lv:synthetic:alpha&b=lv:synthetic:bravo"]) {
      await page.goto(url);
      await expect(page.locator('[data-error="malformed_snapshot"]')).toBeVisible();
      await expect(page.locator(".rank-board, .player-hero, .compare-conclusion")).toHaveCount(0);
    }
  }
});

test("conflicting shared evidence blocks all consumers including the other format", async ({ page }) => {
  await mock(page, r => { r.formats["dynasty-superflex"].entries.find(p => p.player_id === "lv:synthetic:demo-001").facts[0].value.value = 99; });
  for (const url of [board, player("lv:synthetic:demo-001"), compare()]) {
    await page.goto(url);
    await expect(page.locator('[data-error="malformed_snapshot"]')).toBeVisible();
    await expect(page.locator(".rank-board, .player-hero, .compare-conclusion")).toHaveCount(0);
  }
...
test("local-only browser consumers preserve canonical Snapshot identity and history", async ({ page }) => {
  const external = [];
  page.on("request", request => { if (new URL(request.url()).hostname !== "127.0.0.1") external.push(request.url()); });
  await page.route(/^https?:\/\/(?!127\.0\.0\.1)/, route => route.abort());
  for (const url of [board, player("lv:synthetic:demo-001"), compare()]) {
    await page.goto(url);
    await expect(page.locator(".rank-board, .player-hero, .compare-conclusion")).toBeVisible();
  }
  const current = require("../../fixtures/ranking-snapshot-v1/synthetic-current.json");
  const prior = require("../../fixtures/ranking-snapshot-v1/synthetic-prior.json");
  const bytes = S.canonicalizeSnapshot(current);
  const result = await page.evaluate(async ({ bytes, priorCatalog, format, now }) => {
    const S = window.LeagueVectorRankingSnapshotV1;
    const raw = S.parseCanonicalSnapshot(bytes);
    const view = await window.LeagueVectorPilotAdapter.read(bytes, { format, now, priorCatalog });
    return { bytes: S.canonicalizeSnapshot(raw), artifact: await S.snapshotArtifactId(raw), run: view.runId,
      history: view.players.find(p => p.id === "lv:synthetic:charlie").history };
  }, { bytes, priorCatalog: JSON.stringify([S.canonicalizeSnapshot(prior)]), format: "dynasty-1qb", now: Date.parse("2026-09-05T00:00:00Z") });
  expect(result.bytes).toBe(bytes);
  expect(result.artifact).toBe(await S.snapshotArtifactId(current));
  expect(result.run).toBe(current.run_id);
  expect(result.history).toEqual(current.formats["dynasty-1qb"].entries.find(p => p.player_id === "lv:synthetic:charlie").history);
