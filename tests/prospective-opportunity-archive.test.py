...
class ProspectiveArchiveTests(unittest.TestCase):
    ...
    def test_cadence_uses_explicit_2026_phase_boundary(self):
        pre_last = dt.datetime(2026, 9, 10, 0, 19, 59, tzinfo=dt.timezone.utc)
        reg_boundary = dt.datetime(2026, 9, 10, 0, 20, tzinfo=dt.timezone.utc)
        reg_tuesday = dt.datetime(2026, 9, 15, 13, 27, tzinfo=dt.timezone.utc)
        reg_wednesday = dt.datetime(2026, 9, 16, 13, 27, tzinfo=dt.timezone.utc)
        self.assertEqual(runner.season_type_at(pre_last, 2026), "PRE")
        self.assertTrue(runner.should_run_auto(pre_last, 2026))
        self.assertEqual(runner.season_type_at(reg_boundary, 2026), "REG")
        self.assertFalse(runner.should_run_auto(reg_boundary, 2026))
        self.assertTrue(runner.should_run_auto(reg_tuesday, 2026))
        self.assertFalse(runner.should_run_auto(reg_wednesday, 2026))

    def test_regular_cadence_stays_weekly_across_september_october_boundary(self):
        sep_tuesday = dt.datetime(2026, 9, 29, 13, 27, tzinfo=dt.timezone.utc)
        sep_wednesday = dt.datetime(2026, 9, 30, 13, 27, tzinfo=dt.timezone.utc)
        oct_tuesday = dt.datetime(2026, 10, 6, 13, 27, tzinfo=dt.timezone.utc)
        oct_wednesday = dt.datetime(2026, 10, 7, 13, 27, tzinfo=dt.timezone.utc)
        for when in (sep_tuesday, oct_tuesday):
            self.assertEqual(runner.season_type_at(when, 2026), "REG")
            self.assertTrue(runner.should_run_auto(when, 2026))
        for when in (sep_wednesday, oct_wednesday):
            self.assertEqual(runner.season_type_at(when, 2026), "REG")
            self.assertFalse(runner.should_run_auto(when, 2026))

    def test_runner_patches_collector_auto_cadence_to_same_calendar(self):
        september_reg_thursday = dt.datetime(2026, 9, 10, 13, 27, tzinfo=dt.timezone.utc)
        september_reg_tuesday = dt.datetime(2026, 9, 15, 13, 27, tzinfo=dt.timezone.utc)
        self.assertFalse(runner.archive.should_run_auto(september_reg_thursday))
        self.assertTrue(runner.archive.should_run_auto(september_reg_tuesday))
