...
test("approved IDP runtime assets are versioned and loaded in deterministic order", () => {
  ...
  const expected = [
    ...
    "idp-current-season-integration-v01.js?v=0.1",
  ];
  let prior = -1;
...
test("Core adapter builds only the approved current-season contract and never touches dynasty valuation", () => {
  const adapter = fs.readFileSync("idp-current-season-integration-v01.js", "utf8");
  assert.match(adapter, /Rankings\.buildCandidate/);
