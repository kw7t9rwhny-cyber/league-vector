...
Before dispatch, compute `idempotency_identity` as the SHA-256 of the canonical contract with the `idempotency_identity` field omitted. `deriveIdempotencyIdentity` in the library is the normative implementation. The contract must name exact files, not directories or globs; those files must also fit the compiled static safe profile. Every path segment containing `customer` is protected and rejected before any repository mutation, including generic top-level names such as `customer.js`. V0.1 admits only the exact enumerated prohibited-path, prohibited-action, and stop-condition controls in the schema; it rejects extra claimed controls rather than pretending to enforce them. A contract cannot expand the 20-file or 524,288-byte ceilings and must carry the exact supported `$30` provider-cost authority.

The issue's creator, authority actor, authorization timestamps, expiry, repository, current `main` commit and tree, required commands, stop conditions, prohibited actions, QA threshold, exact paths, and duplicate/terminal history are all re-read before inference. Every authority-bearing tagged JSON record is scanned before parsing and rejects duplicate object member names at every depth, including names that become equal after JSON escape decoding. Authority may be at most seven days old and a contract may live at most fourteen days. Missing, moved, broad, stale, contradictory, ambiguous, or unverifiable state fails closed.
...
After the safe-output handler creates the draft PR, a separate deterministic job re-reads the actual bot-authored PR and dispatches exact-head validation through the existing `GITHUB_TOKEN`. It requests GitHub's native dispatch response containing the exact created workflow-run ID, verifies that new run is attempt 1 of the exact validation workflow on the admitted `main` commit and is attributed to `github-actions[bot]`, then writes and reads back one trusted dispatch record binding the task, contract, implementation run/attempt, validation run/attempt, actor, triggering actor, workflow, branch, PR, base, commit, tree, and paths. A reused implementation record blocks another dispatch. Dispatch failure caused by a 401/403 produces `BLOCKED_BY_ADDITIONAL_CREDENTIAL_REQUIREMENT`; missing run details, a second/replayed dispatch, a wrong actor, workflow, branch, or candidate, or any unverifiable provenance fails closed with no weaker fallback.

Validation first proves the unique trusted claim, implementation record, and bot-authored validation-dispatch record; a run that starts before the dispatch record is durable waits only for that record's bounded GitHub readback. It derives the implementation identity from the exact implementation workflow run ID and attempt, requires its own GitHub run ID and attempt to be the single recorded attempt-1 dispatch, binds the runtime actor, triggering actor, event, workflow ref/SHA, candidate branch, PR, base, commit, tree, and paths, verifies both immutable workflow runs, and proves candidate ancestry from the admitted starting commit. Manual, duplicate, replayed, wrong-actor, wrong-workflow, wrong-branch, and substituted runs fail before creator commands execute. It emits `deterministic-evidence.json` with those bindings plus expected and observed identities, patch bytes, exact commands and exit states, `UNKNOWN` test counts when counts are not reliably available, timestamps, and PASS/FAIL. The result is written to both the authoritative work item and candidate PR and an evidence artifact is retained for 30 days.
