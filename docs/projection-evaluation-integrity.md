...
- `frozen_at`, `data_kind`, `consumed_periods`: declare when the final forecast was retained and which periods have already been inspected. The one-season contract represents periods as integer calendar-year numbers from 1 through 9999, matching its four-digit UTC dates. `consumed_periods` must be an array of distinct supported numbers; strings, nulls, containers, booleans, fractions and out-of-range values reject the entire input before comparison with the final season. No coercion is performed. Known real 2020–2025 development years cannot be final holdouts. Synthetic data always yields synthetic evidence, even with disjoint periods.
...
Before reading assessment outcomes, saved forecast validation requires exactly one row for every frozen member, including abstentions: no missing, duplicate or extra canonical identity. Each row must retain the universe's exact alias, position and cohort flags, finite supplied predictions, and matching missing-input disposition. The duplicate final-origin copies must agree. Historical season positions may differ over time without changing identity. If an outcome supplies `position`, this v1 field asserts the frozen cutoff board position and must match it; an omitted position is valid. A realized later position change requires a separately defined field/contract and cannot silently redefine the evaluation board.
...
The saved reader enforces these semantics even for a correctly rechecksummed artifact. Confidence is exactly `{label: "High" | "Medium" | "Low", type: "HEURISTIC"}` with no extra probability fields; artifact claims must match the contract. Every row retains an `intervals` object. An absent target key (including an empty object on abstention) is the v1 no-interval state. A present band must contain exactly numeric finite ordered `p80_low`, `p80_high`, `p90_low`, `p90_high`, `type: "historical_residual_band"`, and `calibrated_probability: false`, with a finite prediction for that target. Null, numeric strings, missing endpoints and extra probability fields fail before coverage arithmetic, even when no later outcome is available. Hash verification does not replace this semantic validation.
