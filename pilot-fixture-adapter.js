...
(function (root, factory) {
  ...
  function read(input, expected = {}) {
    if (!FORMATS.includes(expected.format)) fail("unsupported_format", "Unsupported format. Choose Dynasty 1QB or Dynasty Superflex.");
    if (input == null) fail("no_snapshot", "No snapshot is available.");
    let raw;
    try { raw = S.parseCanonicalSnapshot(typeof input === "string" ? input : S.canonicalizeSnapshot(input)); }
    ...
    if (expected.run != null && expected.run !== raw.run_id) fail("unavailable_run", "This requested run is unavailable. Its ranks have not been replaced with another run.");
    ...
    const state = S.getRunState(raw, new Date(expected.now ?? Date.now()).toISOString());
    ...
    const view = freeze({ runId: raw.run_id, format: expected.format, formats, players: formats[expected.format].players,
      ...
      horizon: S.ASSUMPTIONS[expected.format].horizon, coverage: raw.formats[expected.format].universe.definition,
      assumptions: "12 teams · Full PPR · 1 QB / 2 RB / 3 WR / 1 TE / 2 RB-WR-TE flex. Superflex adds one QB-RB-WR-TE slot. 0.1 rush/receiving yards, 6 rush/receiving TD, 0.04 passing yards, 4 passing TD, −2 interceptions and lost fumbles, +2 successful two-point conversions. " + S.ASSUMPTIONS[expected.format].limitations,
